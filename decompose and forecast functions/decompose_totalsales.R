decompose_totalsales<-function(x, measure){
  ...
  a <- list()
  a$temp<-dygraph(t_data, main = paste("Decomposed trend for total sales by value"))%>%
  ...
  if(length(s_data[1,]) == "0"){
    ...
    a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by value"))%>%
      dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
      dyCrosshair(direction = "vertical")
  }
  return(a)}
      ...
      # tr_data<-supsmu(as.numeric(components$Date),t_data+r_data)
  ...
  a$temp<-dygraph(t_data, main = paste("Decomposed trend for total sales by value"))%>%
    ...
    # a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by value, with a frequncy of ",findfrequency(s_data)))%>%
    #   dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
    #   dyCrosshair(direction = "vertical")
  ...
  a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by value "))%>%
    ...
    a$temp2<-dygraph(t_data+r_data, main = paste("Deseasonalized series for total sales by value (Trend+Remainder)"))%>%
      dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
      dyCrosshair(direction = "vertical")
    return(a)
  }
  ...
  if(measure == 2)
  {
    ...
    x<-x%>%dplyr::select(Date,`Order Qty`)%>%
     group_by(Date)%>%summarise(qty=sum(`Order Qty`))
    ...
    if(length(qty_ma_ts)>60 ){
      ...
      a <- list()
      a$temp<-dygraph(t_data, main = paste("Decomposed trend for total sales by volume"))%>%
        dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
        dyCrosshair(direction = "vertical")
      
      # a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by volume, with frequency ",findfrequency(s_data)))%>%
      #   dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
      #   dyCrosshair(direction = "vertical")
      
      a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by volume "))%>%
        dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
        dyCrosshair(direction = "vertical")

      a$temp2<-dygraph(t_data+r_data, main = paste("Deseasonalized series for total sales by volume (Trend+Remainder)"))%>%
        dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
        dyCrosshair(direction = "vertical")
