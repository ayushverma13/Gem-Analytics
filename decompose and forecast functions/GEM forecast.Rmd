...
map<-read.csv("~/Documents/personal/Courses/R workingdirectory/Gem data/gem_product_code_mapping.csv")
              

map_customer<-read.csv("~/Documents/personal/Courses/R workingdirectory/Gem data/gem_customer_mapping.csv")
...
source("~/Desktop/Data Science technologies project/decompose and forecast functions/forecast_totalsales.R")
a <- forecast_totalsales(master, 1,1)
a$temp2
kable(a$temp1)
...
a <- forecast_totalsales(master, 1,2)
a$temp2
kable(a$temp1)
...
# SALES BY Volume ltr (MONTHLY)

``` {r echo =FALSE, results = 'asis', warning=FALSE, results='show'}
a <- forecast_totalsales(master_ltr, 2,1)
a$temp2
kable(a$temp1)
...
# SALES BY Volume ltr (QUARTERLY)

``` {r echo =FALSE, results = 'asis', warning=FALSE, results='show'}
a <- forecast_totalsales(master_ltr, 2,2)
a$temp2
kable(a$temp1)
```
...
# SALES BY Volume kg (Monthly)

``` {r echo =FALSE, results = 'asis', warning=FALSE, results='show'}
a <- forecast_totalsales(master_kg, 2,1)
a$temp2
kable(a$temp1)
```
...
# SALES BY Volume kg (Quarterly)

``` {r echo =FALSE, results = 'asis', warning=FALSE, results='show'}
a <- forecast_totalsales(master_kg, 2,2)
a$temp2
kable(a$temp1)
```
