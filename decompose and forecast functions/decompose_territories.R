decompose_territory<-function(x, index, measure){
  ...
  if(measure == 1){
    ...
    # x
    territory_value<-x%>%dplyr::select(Territory,Amt.)%>%
    ...
    x<-x%>%dplyr::select(Date,`Order Qty`,Amt.,Territory)%>%rowwise()%>%
      filter( Territory==territory_value[index,1])%>%
    ...
    if(length(amt_ma_ts)>60 ){
      ...
      a <- list()
      a$temp<-dygraph(t_data, main = paste("Decomposed trend for total sales by value for ",territory_value[index,1]))%>%
      ...
      if(length(s_data[1,]) == "0"){
        ...
        a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by value for ",territory_value[index,1]))%>%
          dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
          dyCrosshair(direction = "vertical")
      }
      return(a)}
      ...
      # a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by value, with frequency ",findfrequency(s_data)," for ",territory_value[index,1]))%>%
      #   dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
      #   dyCrosshair(direction = "vertical")
      ...
      a$temp2<-dygraph(t_data+r_data, main = paste("Deseasonalized series for total sales by value (Trend+Remainder), for ",territory_value[index,1]))%>%
        dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
        dyCrosshair(direction = "vertical")
      return(a)
    }
  ...
  if(measure == 2){
    ...
    territory_volume<-x%>%dplyr::select(Territory,`Order Qty`)%>%
      group_by(Territory)%>%summarise(qty_sold=sum(`Order Qty`))%>%
      arrange(desc(qty_sold))%>%
      mutate(cumsum=cumsum(qty_sold),freq=round(qty_sold/sum(qty_sold),3),cum_freq=cumsum(freq))
    
    value <- match("Uncategorized", territory_volume$Territory)
    
    x<-x%>%dplyr::select(Date,`Order Qty`,Amt.,Territory)%>%rowwise()%>%
      filter( Territory==territory_volume[index,1])%>%
    ...
    if(length(qty_ma_ts)>60 ){
      ...
      a <- list()
      a$temp<-dygraph(t_data, main = paste("Decomposed trend for total sales by value for",territory_volume[index,1]))%>%
        dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
        dyCrosshair(direction = "vertical")
      ...
      if(length(s_data[1,]) == "0"){
        ...
        s_data<-s_data%>%xts(order.by = x$Date)
        a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by value for",territory_volume[index,1]))%>%
          dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
          dyCrosshair(direction = "vertical")
      }
      return(a)}
      ...
      a <- list()
      a$temp<-dygraph(t_data, main = paste("Decomposed trend for total sales by volume, for ",territory_volume[index,1]))%>%
        dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
        dyCrosshair(direction = "vertical")
      
      # a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by volume, with frequency ",findfrequency(s_data)," for ",territory_volume[index,1]))%>%
      #   dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
      #   dyCrosshair(direction = "vertical")
      
      a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by volume for ",territory_volume[index,1]))%>%
        dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
        dyCrosshair(direction = "vertical")
      
      a$temp2<-dygraph(t_data+r_data, main = paste("Deseasonalized series for total sales by volume (Trend+Remainder), for ",territory_volume[index,1]))%>%
        dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
        dyCrosshair(direction = "vertical")
