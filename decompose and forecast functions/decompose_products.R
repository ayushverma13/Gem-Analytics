## Malik top products value decompose

decompose_products<-function(x, index, measure){
  ...
  # x
  productgroup_value<-x%>%dplyr::select(product_group,Amt.)%>%
    group_by(product_group)%>%summarise(amt_sold=sum(Amt.))%>%
    arrange(desc(amt_sold))%>%
    mutate(cumsum=cumsum(amt_sold),freq=round(amt_sold/sum(amt_sold),3),cum_freq=cumsum(freq))
  
   x<-x%>%dplyr::select(Date,`Order Qty`,Amt.,product_group)%>%rowwise()%>%
      filter( product_group==productgroup_value[index,1])%>%
   ...
   if(length(amt_ma_ts)>60 ){
     ...
     a <- list()
     a$temp<-dygraph(t_data, main = paste("Decomposed trend for total sales by value for ",productgroup_value[index,1]))%>%
     ...
     if(length(s_data[1,]) == "0"){
       ...
       a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by value for ",productgroup_value[index,1]))%>%
         dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
         dyCrosshair(direction = "vertical")
     }
     return(a)}
     ...
     # a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by value, with frequency ",findfrequency(s_data)," for ",productgroup_value[index,1]))%>%
     #   dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
     #   dyCrosshair(direction = "vertical")
     ...
     a$temp2<-dygraph(t_data+r_data, main = paste("Deseasonalized series for total sales by value (Trend+Remainder), for ",productgroup_value[index,1]))%>%
       dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
       dyCrosshair(direction = "vertical")
     return(a)
   }
  ...
  if(measure == 2){
    ...
    productgroup_volume<-x%>%dplyr::select(product_group,`Order Qty`)%>%
      group_by(product_group)%>%summarise(qty_sold=sum(`Order Qty`))%>%
    ...
    x<-x%>%dplyr::select(Date,`Order Qty`,Amt.,product_group)%>%rowwise()%>%
      filter( product_group==productgroup_volume[index,1])%>%
    ...
    if(length(qty_ma_ts)>60 ){
      ...
      a <- list()
      a$temp<-dygraph(t_data, main = paste("Decomposed trend for total sales by volume for ",productgroup_volume[index,1]))%>%
        dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
        dyCrosshair(direction = "vertical")
      ...
      if(length(s_data[1,]) == "0"){
        ...
        s_data<-s_data%>%xts(order.by = x$Date)
        a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by volume for ",productgroup_volume[index,1]))%>%
          dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
          dyCrosshair(direction = "vertical")
      }
      return(a)}
      ...
      # a$temp1<-dygraph(s_data, main = paste("Decomposed seasonality for total sales by volume, with frequency ",findfrequency(s_data)," for ",productgroup_volume[index,1]))%>%
      #   dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
      #   dyCrosshair(direction = "vertical")
      ...
      a$temp2<-dygraph(t_data+r_data, main = paste("Deseasonalized series for total sales by volume (Trend+Remainder), for ",productgroup_volume[index,1]))%>%
        dyLegend(show = "follow",width = 150)%>%dyOptions(fillGraph = TRUE,drawPoints = TRUE,pointSize = 2)%>%dyRangeSelector()%>%
        dyCrosshair(direction = "vertical")
