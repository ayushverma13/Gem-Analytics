seasonalsales_territory_volume<-function(x, index){
  ...
  productgroup_value<-x%>%select(Territory,Amt.)%>%group_by(Territory)%>%summarise(amt_sold=sum(Amt.))%>%arrange(desc(amt_sold))%>%mutate(cumsum=cumsum(amt_sold),freq=round(amt_sold/sum(amt_sold),3),cum_freq=cumsum(freq))
  ...
  if (index == "unc"){
    if(value > 5){
      product_sales_volume_un<-x%>%select(Date,`Order Qty`,Amt.,Territory)%>%rowwise()%>%filter( Territory=="Uncategorized") 
    ...
    product_sales_volume_1<-x%>%select(Date,`Order Qty`,Amt.,Territory)%>%rowwise()%>%filter( Territory==territory_volume[index,1])
    ...
    a$temp1<-dygraph(product_sales_volume_1_ts, main = paste(index," - Historical sales for",territory_volume[index,1],"by volume"))%>%
