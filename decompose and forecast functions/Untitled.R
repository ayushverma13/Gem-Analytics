forecast_territory<-function ( x, measure, index, horizontype){
  ...
  if(measure== 1){
    ...
    x<-x%>%select(Date,Amt.,Territory)%>%rowwise()%>%filter( Territory==territory_value[index,1])%>%
  ...
  # index<-1
    ...
    territory_volume<-x%>%select(Territory,`Order Qty`)%>%
      group_by(Territory)%>%
    ...
    x<-x%>%select(Date,`Order Qty`,Territory)%>%rowwise()%>%filter( Territory==territory_volume[index,1])%>%
