forecast_territory<-function ( x, measure, index, horizontype){
  ...
  if(measure== 1){
    ...
    x<-x%>%dplyr::select(Date,Amt.,Territory)%>%rowwise()%>%filter(Territory==territory_value[index,1])%>%
    ...
    if(!has_error(HoltWinters(amt_ma_ts,beta = F))){
      #   Holts methods
      output_hw<-HoltWinters(amt_ma_ts,beta = F) #Gamma is setb to false to fit a non-seasonal model
      hw_out_s<-forecast(output_hw,h = h1)
      accuracy_hw<-accuracy(hw_out)
      r_hw_s<-accuracy_hw[3]

      # e<- tsCV(amt_ma_ts, rwf, drift=TRUE, h=1)
      # r_hw<-sqrt(mean(e^2, na.rm = TRUE))
    }else{
      r_hw_s<-Inf
    }
    ...
    if(!has_error(HoltWinters(amt_ma_ts,beta = F,gamma = F))){
      #   Holts methods
    output_hw1<-HoltWinters(amt_ma_ts,beta = F,gamma = F) #Gamma is setb to false to fit a non-seasonal model
    hw_out<-forecast(output_hw1,h = h1)
    accuracy_hw<-accuracy(hw_out)
    r_hw<-accuracy_hw[3]
    ...
    if(!has_error(HoltWinters(amt_ma_ts_d,beta = F,gamma = F))){
      #   Holts methods
      output_hw<-HoltWinters(amt_ma_ts_d,beta = F,gamma = F) #Gamma is setb to false to fit a non-seasonal model
      hw_out_d<-forecast(output_hw1,h = h1)
      hw_out_d$mean<-hw_out_d$mean+tail(components$season,12)
      hw_out_d$lower[,1]<-hw_out_d$lower[,1]+tail(components$season,12)
      hw_out_d$lower[,2]<-hw_out_d$lower[,2]+tail(components$season,12)
      hw_out_d$upper[,1]<-hw_out_d$upper[,1]+tail(components$season,12)
      hw_out_d$upper[,2]<-hw_out_d$upper[,2]+tail(components$season,12)
      accuracy_hw<-accuracy(hw_out_d)
      r_hw_d<-accuracy_hw[3]
    ...
    v_min<-c("r_arima"= r_arima,"r_drift"=r_drift,"r_meanf"=r_meanf,"r_naive"=r_naive,"r_ets"=r_ets,"r_hw"=r_hw,"r_arima.seasonal"= r_arima_s,"r_drift.seasonal"=r_drift_s,"r_naive.seasonal"=r_naive_s,"r_ets.seasonal"=r_ets_s,"r_hw.seasonal"=r_hw_s,"r_arima.decomposed"= r_arima_d,"r_drift.decomposed"=r_drift_d,"r_meanf.decomposed"=r_meanf_d,"r_naive.decomposed"=r_naive_d,"r_ets.decomposed"=r_ets_d,"r_hw.decomposed"=r_hw_d)
    ...
    if(model[2] == "hw.seasonal"){
      model_final = hw_out_s}
    ...
    if(model[2] == "hw.decomposed"){
      model_final = hw_out_d}
    ...
    if(horizontype==1){
      ...
      temp$temp2<-dygraph(df_final, main = paste("total sales by value for ",territory_value[index,1]))%>%
  ...
  # index<-1
    ...
    dh<-head(x$Date, n=1)
    d<-tail(x$Date, n=1)
    territory_volume<-x%>%dplyr::select(Territory,`Order Qty`)%>%
      group_by(Territory)%>%
    ...
    x<-x%>%dplyr::select(Date,`Order Qty`,Territory)%>%rowwise()%>%filter( Territory==territory_volume[index,1])%>%
    ...
    if(horizontype==1){
      ...
      temp$temp2<-dygraph(df_final, main = paste("total sales by volume for ",territory_volume[index,1]))%>%
